...
pub fn render_session_picker(state: &SessionPickerState, frame: &mut Frame, area: Rect, theme: &impl Theme) {
    if !state.active {
        return;
    }
...
fn render_help_bar(frame: &mut Frame, area: Rect, theme: &impl Theme) {
    let help_text =
